...
case "$MODE" in
  ...
  new)
    ...
    if [ -z "$NPP_BIN" ]; then
      ...
      make -C PowerEditor/gcc -f makefile CXX=x86_64-w64-mingw32-g++ CC=x86_64-w64-mingw32-gcc AR=x86_64-w64-mingw32-ar RC=x86_64-w64-mingw32-windres PREBUILD_EVENT_CMD="true" -j$(nproc) 2>&1 | tee -a "$TEST_LOG"
